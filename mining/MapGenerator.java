...
public class MapGenerator {
    ...
    public void init(){
        ...
        // random number is connected to the outer edges, so all together there are 2 * (x + y) values
        int randomNum = ThreadLocalRandom.current().nextInt(0, (x + y) * 2);
        while(randomNum == 0 || randomNum == y - 1 || randomNum == y || randomNum == x + y - 1 || randomNum == x + y || randomNum == x + 2 * y - 1
                || randomNum == x + 2 * y || randomNum == 2 * x + 2 * y - 1){
            randomNum = ThreadLocalRandom.current().nextInt(0, (x + y) * 2);
        }

        // set up entrance
        // the commented code gives back the block in the wall, the non commented gives back the one inside by 1 block
        if(randomNum < y){
    ...
    public void create_tunnels(){
        ...
        // while there is at least one unchecked non wall cave block it tries to generate in the 4 direction, if it is not a wall or cave
        while(checkloop()){
            for(int i = 0; i < x; i++){
                for(int j = 0; j < y; j++){
